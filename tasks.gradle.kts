...
tasks {
    ...
    task("createModule") {
        doFirst {
            ...
            val createFeatureModuleClasses = type == ModuleType.FEATURE && !screenName.isNullOrEmpty()
            val modulePath = "${type.path}/$nName/src/main/java/com.justparokq.$nName"
            val namespace = "com.justparokq.${type.path}.$nName"

            logTaskParams(type.path, nName, createTestFolder, screenName)
...
fun createBaseModuleClasses(
    ...
    if (createTestFolder) {
        val testModulePath = "${moduleType.path}/$featureName/src/test/java/com/justparokq/$featureName"
...
fun createFilesFromTemplate(newFilePath: String, templatePath: String, screenName: String, namespace: String) {
